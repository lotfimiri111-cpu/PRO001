...
def _hdr(slide, T, title, sub='', side='right', slide_num=None, total_slides=13, req=None):
    ...
    patch_header_rtl(slide, T, _FONT, title, sub, slide_num, total_slides, HEADER_H)
# ── محتوى Y وH ─────────────────────────────────────────────────────────
CY0 = HEADER_H + 0.3
...
def _make_cover_old(prs, req: PresentationRequest, T: Theme):
...
def make_stats(prs, req: PresentationRequest, T: Theme):
    ...
    patch_header_rtl(slide, T, _FONT, "الأرقام والإحصاءات الرئيسية", "", 7,
                     getattr(req, '_total_slides', 13), HEADER_H)
    CY = CY0; CH = _ch()
    if not req.stats: return slide
    hero_stats(slide, T, _FONT, req.stats, 1.0, CY, W - 2.0, CH)
    return slide

def _make_stats_old(prs, req: PresentationRequest, T: Theme):
    slide = blank_slide(prs); _bg(slide,T,'a')
...
def _make_final_old(prs, req: PresentationRequest, T: Theme):
