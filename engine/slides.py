...
from engine.primitives import (
    ...
    multi_stop_gradient, glow, inner_glow, reflection,
    diamond, hexagon, decorative_dots,
    card_3d, glass_card, shimmer_rect,
    icon_circle, number_badge, slide_number,
    accent_line, section_header_bar, divider,
...
def _hdr(slide, T, title, sub='', side='right', slide_num=None, total_slides=13, req=None):
    ...
    txt(slide, title, title_x, 0.2, title_w, HEADER_H*0.62,
        ...
        rtl=True, vcenter=True, line_spacing=1.1, letter_spacing=10)

    # خط تزييني أسفل العنوان
    accent_line(slide, title_x, HEADER_H*0.68, title_w*0.55, T, style='glow', thickness=0.05)
...
def make_cover(prs, req: PresentationRequest, T: Theme):
    ...
    accent_line(slide,cx+cw*0.12,title_y+title_h*0.91,cw*0.76,T,style='glow',thickness=0.06)
    ...
    for i,(lbl,val) in enumerate(rows):
        y=info_y+i*rh
        rb=shimmer_rect(slide,cx+0.25,y+0.04,cw-0.62,rh-0.08,T,radius=8)
...
def make_intro(prs, req: PresentationRequest, T: Theme):
    ...
    for i,(icon,lbl,val) in enumerate(items[:2]):
        ...
        # شريط علوي ملوّن
        tp=section_header_bar(slide,x,card_y,col_w,0.42,
                                 label='', T=T)
...
def make_plan(prs, req: PresentationRequest, T: Theme):
    ...
    for i,ch in enumerate(chapters):
        ...
        even=i%2==0
        # الصف
        rw=shimmer_rect(slide,1.0,y,W-2.0,row_h,T,radius=9)
        if rw:
...
def make_objectives(prs, req: PresentationRequest, T: Theme):
    ...
    for i,(lbl,items) in enumerate(cols):
        x=1.0+i*(col_w+gap)
        cc=card_3d(slide,x,CY,col_w,CH,T,radius=12)
        if cc: glow(cc,T.accent.lstrip('#'),radius=14,alpha=0.09)
        # هيدر العمود
        hh=0.74
...
def make_importance(prs, req: PresentationRequest, T: Theme):
    ...
    for i,item in enumerate(items):
        ...
        cc=glass_card(slide,x,y,col_w,card_h,T,radius=12)
