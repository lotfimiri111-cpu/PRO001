...
from engine.primitives import (
    ...
    diamond, decorative_dots,
    card_3d, glass_card, shimmer_rect,
    slide_number, accent_line, section_header_bar, divider,
...
def _header(slide, T: Theme, title: str, page_num: int = 0, req=None):
    ...
    txt(slide, title, title_x, 0.22, title_w, HEADER_H-0.55,
        font=_FONT, size=SZ_SLIDE_TITLE, bold=True,
        color=T.text_light_rgb, align=PP_ALIGN.RIGHT, rtl=True, vcenter=True,
        letter_spacing=10)
    accent_line(slide, title_x, HEADER_H*0.70, title_w*0.5, T,
                style='glow', thickness=0.05)
...
def make_cover(prs, req: PresentationRequest, T: Theme):
    ...
    accent_line(slide,W*0.12,info_y-0.18,W*0.76,T,style='double',thickness=0.06)
...
def make_intro(prs, req: PresentationRequest, T: Theme):
    ...
    for i, (lbl, val) in enumerate(items[:2]):
        y = cy + i * (card_h + 0.25)
        # بطاقة خلفية
        cb = shimmer_rect(slide, cx, y, cw - 0.4, card_h, T, radius=9)
        # خط accent يميني
...
def make_plan(prs, req: PresentationRequest, T: Theme):
    ...
    for i, ch in enumerate(chapters):
        y = CY + i*(row_h+gap)
        fill = T.bg2_rgb if i%2==0 else T.card_rgb
        rw = shimmer_rect(slide, MX, y, W-MX*2, row_h, T, radius=8)
...
def make_conclusion(prs, req: PresentationRequest, T: Theme):
    ...
    cb = glass_card(slide, MX, CY, cw, CH, T, radius=10)
...
def make_final(prs, req: PresentationRequest, T: Theme):
    ...
    cy = H*0.1; ch = H*0.8
    cb = glass_card(slide, MX, cy, cw, ch, T, radius=12)
    if cb: glow(cb, T.accent.lstrip('#'), radius=36, alpha=0.16)
