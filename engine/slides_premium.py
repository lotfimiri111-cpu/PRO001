...
from engine.primitives import (
    ...
    number_badge, icon_circle, slide_number,
...
def _sidebar(slide, T, icon, label1, label2=""):
    ...
    txt(slide, label1, 0.2, label_y, SW-0.4, 1.1,
        ...
        letter_spacing=8)
    accent_line(slide, 0.3, label_y+1.15, SW-0.6, T,
                style='gradient', thickness=0.05)
    if label2:
...
def make_cover(prs, req: PresentationRequest, T: Theme):
    ...
    accent_line(slide,mcx+mcw*0.08,title_y+title_h*0.92,mcw*0.84,T,style='glow',thickness=0.06)

    ic=glass_card(slide,mcx,info_y,mcw,info_h,T,radius=12)
    ...
    for i,(lbl,val) in enumerate(rows):
        y=info_y+i*rh
        rb=shimmer_rect(slide,mcx+0.25,y+0.04,mcw-0.62,rh-0.08,T,radius=8)
